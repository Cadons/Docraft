...
namespace docraft::renderer::painter {
    template<typename TShapeNode>
    void draw_polygonal_shape(const TShapeNode &shape_node,
        ...
        const auto &bg_color = shape_node.background_color().toRGB();
        const auto &border_color = shape_node.border_color().toRGB();
        const float border_width = shape_node.border_width();
        if (bg_color.a <= 0.0F && (border_width <= 0.0F || border_color.a <= 0.0F)) {
            return;
        }

        const auto &points = shape_node.points();
        if (points.size() < 3U) {
            return;
        }
        ...
        const auto &origin = shape_node.position();
