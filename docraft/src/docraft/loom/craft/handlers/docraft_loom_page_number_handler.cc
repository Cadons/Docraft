...
namespace docraft::loom::craft {
    std::shared_ptr<nodes::DocraftLoomNode> DocraftLoomPageNumberHandler::build(
        const docraft::craft::DocraftParsedElement& element, DocraftLoomTableHandlerContext& context)
    {
        const auto& data = std::any_cast<const docraft::craft::parser::ParsedPageNumberData&>(element.data);
        auto node = std::make_shared<nodes::DocraftLoomPageNumber>();
        context.fill_page_number_node(*node, data);
