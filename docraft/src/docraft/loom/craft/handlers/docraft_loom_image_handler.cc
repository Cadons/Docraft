...
namespace docraft::loom::craft {
    std::shared_ptr<nodes::DocraftLoomNode> DocraftLoomImageHandler::build(
        const docraft::craft::DocraftParsedElement& element, DocraftLoomTableHandlerContext& context)
    {
        const auto& data = std::any_cast<const docraft::craft::parser::ParsedImageData&>(element.data);
        auto node = std::make_shared<nodes::DocraftLoomImage>();
        context.fill_image_node(*node, data);
