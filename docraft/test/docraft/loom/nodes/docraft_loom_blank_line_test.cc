...
namespace docraft::test {
    ...
    TEST_F(DocraftLoomBlankLineTest, LayoutStretchesToPageWidthAndAdvancesCursor)
    {
        ...
        EXPECT_FLOAT_EQ(blank_line->layout_box().frame(docraft::test::utils::LayoutBoxTestAccess::make_layout_proof()).size.width, 200.0F);
        EXPECT_FLOAT_EQ(blank_line->layout_box().frame(docraft::test::utils::LayoutBoxTestAccess::make_layout_proof()).size.height, 3.0F);
        ...
        EXPECT_FLOAT_EQ(next->layout_box().frame(docraft::test::utils::LayoutBoxTestAccess::make_layout_proof()).position.y,
                        blank_line->layout_box().frame(docraft::test::utils::LayoutBoxTestAccess::make_layout_proof()).position.y + 3.0F);
    }
    ...
    TEST_F(DocraftLoomBlankLineTest, LayoutWithDefaultConstructedProcessorHasZeroWidth)
    {
        ...
        EXPECT_FLOAT_EQ(blank_line->layout_box().frame(docraft::test::utils::LayoutBoxTestAccess::make_layout_proof()).size.width, 0.0F);
    }
    ...
    TEST_F(DocraftLoomBlankLineTest, AbsolutePositionDoesNotAdvanceCursor)
    {
        ...
        EXPECT_FLOAT_EQ(blank_line->layout_box().frame(docraft::test::utils::LayoutBoxTestAccess::make_layout_proof()).position.x, 42.0F);
        EXPECT_FLOAT_EQ(blank_line->layout_box().frame(docraft::test::utils::LayoutBoxTestAccess::make_layout_proof()).position.y, 42.0F);

        auto next = std::make_shared<loom::nodes::DocraftLoomBlankLine>();
        next->accept(*measure_);
        next->accept(layout);
        // Untouched by the absolute node: still at the cursor's default top margin.
        EXPECT_FLOAT_EQ(next->layout_box().frame(docraft::test::utils::LayoutBoxTestAccess::make_layout_proof()).position.y, 10.0F);
    }
} // namespace docraft::test