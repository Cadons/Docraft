...
namespace docraft::test::loom::nodes {
	...
	TEST(DocraftLoomNodeTest, StartsWithNoChildrenAndZeroInitializedLayoutBox)
	{
		...
		EXPECT_EQ(node.layout_box().page_index, 0);
		EXPECT_EQ(node.layout_box().frame.position.x, 0.0F);
		EXPECT_EQ(node.layout_box().frame.position.y, 0.0F);
		EXPECT_EQ(node.layout_box().frame.size.width, 0.0F);
		EXPECT_EQ(node.layout_box().frame.size.height, 0.0F);
	}
	...
	TEST(DocraftLoomNodeTest, PersistsLayoutBoxEdits)
	{
		...
		node.edit_layout_box().page_index = 3;
		node.edit_layout_box().frame.position.x = 10.0F;
		node.edit_layout_box().frame.position.y = 12.0F;
		node.edit_layout_box().frame.size.width = 50.0F;
		node.edit_layout_box().frame.size.height = 60.0F;

		EXPECT_EQ(node.layout_box().measured_size.width, 42.0F);
		EXPECT_EQ(node.layout_box().measured_size.height, 24.0F);
		EXPECT_EQ(node.layout_box().page_index, 3);
		EXPECT_EQ(node.layout_box().frame.position.x, 10.0F);
		EXPECT_EQ(node.layout_box().frame.position.y, 12.0F);
		EXPECT_EQ(node.layout_box().frame.size.width, 50.0F);
		EXPECT_EQ(node.layout_box().frame.size.height, 60.0F);
	}
} // namespace docraft::test::loom::nodes
